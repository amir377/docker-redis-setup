...
# Prompt user for Redis setup details
container_name=$(prompt_user "Enter the container" "redis")
...
echo "Starting Docker Compose with --build for Redis..."
if docker-compose up -d --build; then
    echo "Checking container status..."
    if [ "$(docker inspect -f '{{.State.Running}}' $container_name)" = "true" ]; then
        echo "Redis setup is complete and running. Connecting to the container..."
